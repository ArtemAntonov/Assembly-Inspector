...
class NN(nn.Module):
    def __init__(self, img_channels: int):
        ...
        self.layers = mobilenet_v3_small(weights="MobileNet_V3_Small_Weights.IMAGENET1K_V1")        
        ...
        self.layers.classifier[3] = nn.Linear(1024, 1)
...
class Trainer:
    ...
    def get_batch_size(self, img_shape: tuple, ds: ImageFolder, train_idx: list) -> int:
        ...
        #):
            if self.device == "cuda":
                ...
                if self.batch_size_props["batch_size"] == 2:
                    ...
                    if not test_res[0]:
                        ...
                        batch_size = min(batch_size, BATCH_SIZE_LIMIT)  # batch size bigger than 256 can be too big
            ...
            if self.batch_size_props["batch_size"] > len(train_idx) // 4:
                batch_size = len(train_idx) // 4
            else:
